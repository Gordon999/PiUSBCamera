...
# camera resolution for taking pictures (for pi camera v1 set 2592 and 1944)
width  = 1280
height = 960
...
for x in range(0,len(config)):
    ...
    for y in range(0,len(fet)):
        ...
        if fet[y][0:3] == "val":
            valu = fet[y][6:]
        if valu != -1 and defa != -1 and name[0:4] != "h264" and name[0:4] != "vide" and name[0:4] != "repe" and name[0:4] != "colo"  and name[0:4] != "comp" and name != "exposure_dynamic_framerate": 
...
text(0,1,0,1,"CAPTURE STILL",ft,7)
l = len(parameters)
