           ...
           if mousex > cwidth:
               ...
               if g == (int(cheight/bh)-1)*2:
                   ...
                   if mousex < cwidth + int(bw/2):
                       ...
                       now = datetime.datetime.now()
                       timestamp = now.strftime("%y%m%d%H%M%S")
                       output = cv2.VideoWriter("/home/pi/Videos/" + timestamp + ".mp4", vid_cod, 20.0, (vwidth,vheight))
                       stop = 0
                       time.sleep(2)
                       while stop == 0:
                           for event in pygame.event.get():
                           ...
                           imagep = pygame.surfarray.make_surface(frame2)
                           imagep = pygame.transform.rotate(imagep,270)
                           imageq = pygame.transform.scale(imagep, [cwidth,cheight])
                           imageq = pygame.transform.flip(imageq, True, False)
